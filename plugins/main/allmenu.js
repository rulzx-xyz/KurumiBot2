...
function getContextInfo(botConfig, m, thumbBuffer) {
  const saluranId = botConfig.saluran?.id || "120363406203774227@newsletter";
  const saluranName =
...
async function handler(m, { sock, config: botConfig, db, uptime }) {
  ...
  try {
    switch (allmenuVariant) {
      ...
      case 2:
        const media = await prepareWAMessageMedia({
          image: fs.readFileSync(config.assets["kurumi"])
      ...
      case 5: {
        ...
        const thumbnail = await _sharp(fs.readFileSync(config.assets["kurumi"])).resize(300, 300).toBuffer()
        const qOrder = {
        ...
        const media4 = await prepareWAMessageMedia({ video: fs.readFileSync(config.assets["kurumiv3"]), gifPlayback: true }, { upload: sock.waUploadToServer });
            ...
            message: {
              ...
              interactiveMessage: {
                ...
                contextInfo: {
                  ...
                  forwardingScore: 9,
                  forwardedNewsletterMessageInfo: {
                    newsletterJid: config.saluran?.id || "120363406203774227@newsletter",
                    newsletterName: config.saluran?.name || config.bot?.name || "kurumi-AI",
                    serverMessageId: 127,
                  },
                },
                nativeFlowMessage: {
              ...
              interactiveMessage: {
                header: {
                  ...
                  locationMessage: {
                    degreesLatitude: 0,
                    degreesLongitude: 0,
                    name: config.bot?.name || "kurumi-AI",
    ...
    if (audioEnabled) {
      const audioUrl = botConfig.assets["kurumi-mp3"];
      ...
      try {
        ...
        switch (audioVariant) {
          ...
          case 3: {
            ...
            await sock.sendMessage(m.chat, {
              audio: fs.readFileSync(config.assets["kurumi-mp3"]),
              mimetype: "audio/mpeg",
              ptt: false,
            }, { quoted: qtext });
            break;
